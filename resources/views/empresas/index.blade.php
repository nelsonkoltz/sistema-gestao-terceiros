...
<div class="listing-page">
 ...
 <section class="listing-card">
  ...
  @if($empresas->count())
   <div class="table-wrapper"><table><thead><tr><th>Empresa</th><th>CPF / CNPJ</th><th>Telefone</th><th>E-mail</th><th><span class="sr-only">Ações</span></th></tr></thead><tbody>
   @foreach($empresas as $empresa)<tr><td class="primary-cell"><a href="{{ route('empresas.show',$empresa) }}">{{ $empresa->nome }}</a></td><td class="nowrap">{{ $empresa->cnpj }}</td><td class="nowrap">{{ $empresa->telefone }}</td><td>{{ $empresa->email }}</td><td><div class="actions"><a href="{{ route('empresas.show',$empresa) }}" class="icon-btn view" title="Detalhes"><i class="bi bi-eye"></i></a>@if(auth()->user()->permissao !== 'Consulta')<a href="{{ route('empresas.edit',$empresa) }}" class="icon-btn edit" title="Editar"><i class="bi bi-pencil"></i></a><form action="{{ route('empresas.destroy',$empresa) }}" method="POST" onsubmit="return confirm('Deseja excluir esta empresa e seus registros vinculados?')">@csrf @method('DELETE')<button class="icon-btn delete" title="Excluir"><i class="bi bi-trash"></i></button></form>@endif</div></td></tr>@endforeach
