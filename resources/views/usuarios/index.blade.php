...
<div class="listing-page">
 ...
 <section class="listing-card">
  ...
  @if($usuarios->count())
   <div class="table-wrapper"><table><thead><tr><th>Usuário</th><th>Login</th><th>Setor</th><th>Permissão</th><th>Status</th><th><span class="sr-only">Ações</span></th></tr></thead><tbody>
   @foreach($usuarios as $usuario) @php $permissionClass=$usuario->permissao==='Administrador'?'administrator':($usuario->permissao==='Consulta'?'readonly':'operator'); @endphp
    <tr><td class="user-cell"><span class="user-avatar">{{ mb_strtoupper(mb_substr($usuario->name,0,1)) }}</span><span><a href="{{ route('usuarios.show',$usuario) }}">{{ $usuario->name }}</a>@if(auth()->id()===$usuario->id)<small>Você</small>@endif</span></td><td><span class="username">{{ $usuario->username }}</span></td><td>{{ $usuario->setor?:'—' }}</td><td><span class="permission {{ $permissionClass }}"><i class="bi bi-shield-check"></i>{{ $usuario->permissao }}</span></td><td><span class="status {{ $usuario->ativo ? 'status-finalizado' : 'status-cancelado' }}">{{ $usuario->ativo ? 'Ativa' : 'Inativa' }}</span></td><td><div class="actions"><a href="{{ route('usuarios.show',$usuario) }}" class="icon-btn view" title="Detalhes"><i class="bi bi-eye"></i></a><a href="{{ route('usuarios.edit',$usuario) }}" class="icon-btn edit" title="Editar"><i class="bi bi-pencil"></i></a>@if(auth()->id()!==$usuario->id)<form action="{{ route('usuarios.destroy',$usuario) }}" method="POST" onsubmit="return confirm('Deseja excluir este usuário?')">@csrf @method('DELETE')<button class="icon-btn delete" title="Excluir"><i class="bi bi-trash"></i></button></form>@endif</div></td></tr>
